...
def n_most_common(pix):
    assert isinstance(pix, list), 'Must be a list type'
    if len(pix) == 0:
        return None
    first = Counter(pix).most_common(n=5)[0][0]
    if first == (0, 0, 0) or (255, 255, 255):
        return Counter(pix).most_common(n=5)[1][0]
    return Counter(pix).most_common(n=5)[0][0]

def pixel_list(img, wr, hr):
    pixel = []
    for x in range(int(0.35*wr), int(0.65*wr)):
        for y in range(int(0.35*hr), int(0.65*hr)):
            r, g, b = img.getpixel((x, y))
            pixel.append((r, g, b))
    return pixel

def detect_color(pr, pg, pb): # n_most_common에서 나온 값을 매칭
    wb = openpyxl.load_workbook("colorName.xlsx")  # 새로 정의한 색상명
    sheet = wb.active

    colors_basic_dic = {}
    for i in range(2, 970):
        colors_basic_dic[sheet['A'+str(i)].value] = sheet['B'+str(i)].value
    ...
    peaked_rgb = np.asarray([int(str(hex(pr)), 16), int(str(hex(pg)), 16), int(str(hex(pb)), 16)], np.uint8)
    ...
    print(peaked_color_name)
    newColor = colors_basic_dic[peaked_color_name]
...
def third_detection(categoryName, x_test):
    ...
    model = load_model('model_lsleeve_'+categoryName+'.h5')
...
def second_detection(categoryName, x_test):
    ...
    model = load_model('model_'+categoryName+'.h5')
    prediction = model.predict(x_test)
    return getCatName(classNames, prediction)
...
def detect_category(x_test):
    ...
    if catName != 'skirt':
        ...
        if catName == ('sleeve' or 'haveCollar' or 'padding'):
            catName = third_detection(catName, x_test)
    return catName


def pants_length(x_test):
    classNames = ['lpants', 'spants']
    model = load_model('modelPants.h5')
    prediction = model.predict(x_test)
    return getCatName(classNames, prediction)
